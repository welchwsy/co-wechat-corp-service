 ...
 */
  ...
  async request(url, opts, retry = 3) {
    ...
    if (data && data.errcode) {
      ...
      if ((err.code === 40001 || err.code === 42001) && retry > 0 && !this.tokenFromCustom) {
        ...
        let token = await this.getSuiteToken();
        let urlobj = liburl.parse(url, true);

        if (urlobj.query && urlobj.query.suite_access_token) {
          urlobj.query.suite_access_token = token.suiteAccessToken;
  ...
  async preRequst(url, opts, retry = 3) {
    ...
    let urlobj = liburl.parse(url, true);
    if (urlobj.query) {
      urlobj.query.suite_access_token = this.suiteToken.suiteAccessToken;
    }
    return await this.request(liburl.format(urlobj), opts, retry);
  }
